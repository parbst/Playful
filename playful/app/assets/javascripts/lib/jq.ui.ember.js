...
JQ.AutoComplete = Em.Component.extend(JQ.Widget, Ember.TextSupport, {
  uiType: 'autocomplete',
  tagName: 'input ',
  attributeBindings: ['value'],
  uiOptions: ['delay', 'disabled', 'minLength', 'source', 'position'],
  uiEvents: ['change', 'focus', 'search', 'response', 'select'],
  query: null,
  selected: null,
  ...
  select: function(event, item){
    this.set('selected', item.item)
  },
  showSuggestions: function(){
    // defer so the observer updating the widget is executed first
    setTimeout(_.bind(function(){
      if(this.get('autoShowSuggestions')) {
        this.get('ui').search();
      }
    }, this), 0);
  }.observes('source'),
  selectedDidChange: function(){
    var selected = this.get('selected'), value = '';
    if(selected){
    ...
    this.get('ui').element[0].value = value;
  }.observes('selected'),
  ...
  keyUp: function(e){
      if (e.keyCode == 13){
          ...
          console.log("JQ.AutoComplete ENTER KEY")
      }
  }
});
